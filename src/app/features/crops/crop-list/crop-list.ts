...
import { SigmaTableComponent, SigmaCellDirective, SigmaColumn } from '../../../shared/components/sigma-table/sigma-table';
...
@Component({
  ...
  imports: [
    ...
    SigmaSpinnerComponent, SigmaInputComponent, SigmaTableComponent, SigmaCellDirective, CropEventTypesDialogComponent,
  ],
...
export class CropListComponent implements OnInit, OnDestroy {
  crops: Crop[] = [];
  newCrop: Partial<Crop> = { name: '', description: '', estimatedGrowthDays: 0, inactivityDaysThreshold: 0 };
  ...
  readonly cropColumns = [
    ...
    { key: 'recommendedFertilizationDays', labelKey: 'crops.recommendedFertilizationDays', class: 'tc' },
    { key: 'recommendedPestControlDays', labelKey: 'crops.recommendedPestControlDays', class: 'tc' },
  ...
  get formErrors(): string[] {
    ...
    const errors: string[] = [];
    if (!this.newCrop.name || this.newCrop.name.trim().length < 2) errors.push('crops.errors.nameRequired');
    if (!this.newCrop.estimatedGrowthDays || this.newCrop.estimatedGrowthDays < 1) errors.push('crops.errors.growthDaysRequired');
    if (!this.newCrop.inactivityDaysThreshold || this.newCrop.inactivityDaysThreshold < 1) errors.push('crops.errors.inactivityRequired');
    return errors;
  }
  ...
  get isEditing(): boolean { return this.editingCropId !== null; }
  ...
  loadCrops(): void {
    ...
    this.cropService.getAll().subscribe({
      next: (data) => { this.crops = data; this.loading = false; this.cdr.markForCheck(); },
  ...
  startEdit(crop: Crop): void { this.editingCropId = crop.id; this.newCrop = { ...crop }; this.submitted = false; }
  cancelEdit(): void { this.editingCropId = null; this.resetForm(); }
  ...
  openEventTypesDialog(cropId: number, cropName: string): void {
    ...
    forkJoin({
      ...
      error: () => {
        ...
        this.snackBar.open('Error al cargar tipos de evento', 'Cerrar', { duration: 4000, panelClass: 'snack-error' });
        this.cdr.markForCheck();
      },
    });
  }
  ...
  onDialogClose(result: Set<number> | null): void {
    this.dialogVisible = false;
    if (!result) return;
    const toAdd = this.dialogAllEventTypes.filter(et => result.has(et.id) && !this.dialogAssignedIds.has(et.id));
    const toRemove = this.dialogAllEventTypes.filter(et => !result.has(et.id) && this.dialogAssignedIds.has(et.id));
    ...
    forkJoin(operations).subscribe({
      next: () => this.snackBar.open('Tipos de evento actualizados exitosamente', 'Cerrar', { duration: 3000, panelClass: 'snack-success' }),
      error: () => this.snackBar.open('Error al actualizar tipos de evento', 'Cerrar', { duration: 4000, panelClass: 'snack-error' }),
    });
  }

  resetForm(): void {
    this.submitted = false;
    this.newCrop = { name: '', description: '', estimatedGrowthDays: 0, inactivityDaysThreshold: 0, irrigationFrequencyHours: 0, recommendedFertilizationDays: 0, recommendedPestControlDays: 0 };
  }
}
